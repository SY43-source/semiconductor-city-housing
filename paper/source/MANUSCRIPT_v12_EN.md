...
**(5) The planning-horizon trajectory (§4.7-(5)) was recomputed after correction and is consistent with §4.4**, but cohort-maturation scenario B is a simple assumption that shifts the age profile by one five-year band and does not model mortality, out-migration, or re-entry separately. The correction coefficient <i>λ</i> is also held fixed across the planning horizon; if Godeok's headship and household-size structure themselves converge toward the Pyeongtaek average, <i>λ</i> should shrink, in which case the 2035 single-person share would be higher than this study's estimate (12.9%). Annual projections depend on the recent three-year slope, so changes to campus expansion schedules require re-estimation, and post-saturation values are lower bounds.
